...
def equal(u,v):
    ...
    """
    assert u.D == v.D
    for i in (u.f.keys() & v.f.keys()):
        if u.f[i] != v.f[i]:
            return False
    for i in (u.f.keys() - v.f.keys()):
        if u.f[i] != 0:
            return False
    for i in (v.f.keys() - u.f.keys()):
        if v.f[i] != 0:
            return False
    return True
...
def add(u,v):
    ...
    Make sure to add together values for all f.keys from u.f and v.f even if some f.keys in u.f do not
...
def scalar_mul(v, alpha):
    ...
    True
    """
    return Vec(v.D, {key : v[key] * alpha for key in v.f.keys()})
