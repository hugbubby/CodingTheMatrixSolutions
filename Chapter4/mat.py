...
def vector_matrix_mul(v, M):
    ...
    r = Vec(M.D[1], {})
    for k in v.f.keys():
        r += v.f[k] * Vec(M.D[1], {m_k[1] : M.f[m_k] for m_k in M.f.keys() if m_k[0] == k})
...
def matrix_vector_mul(M, v):
    ...
    r = Vec(M.D[0], {})
    for k in v.f.keys():
        r += v.f[k] * Vec(M.D[0], {m_k[0] : M.f[m_k] for m_k in M.f.keys() if m_k[1] == k})
    return r
...
def matrix_matrix_mul(A, B):
    ...
    ret = Mat((A.D[0], B.D[1]), {})
    for row in ret.D[0]:
        row_vec = Vec(A.D[1], {key[1] : A[key] for key in A.f.keys() if key[0] == row})
        row_result = row_vec * B
        for column in ret.D[1]:
            if column in row_result.f:
                ret.f[(row, column)] = row_result.f[column]
    return ret
